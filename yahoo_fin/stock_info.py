...
def get_growth_tech_stocks():
    '''return grouwth stock, refresh yearly'''

    tables = pd.read_html("https://finance.yahoo.com/screener/predefined/growth_technology_stocks?offset=0&count=100")
    result = tables[0]
    return result

def get_undervalued_growth_stocks():
    ''' return undervalued growth stocks, refresh yearly'''
    tables = pd.read_html("https://finance.yahoo.com/screener/predefined/undervalued_growth_stocks?offset=0&count=100")
    result = tables[0]
    return result
